...
public:
	...
	Walls(glm::mat4 M,  ShaderProgram* sp,GLuint tex,float roomWidth,float roomHeight)
	{
		...
		this->translates = { glm::vec3(roomWidth/2.0f-doorSize,roomHeight/2.0f-2.0f,0.0f),
			vec3(-roomWidth/2.0f+doorSize,roomHeight/2.0f-2.0f,0.0f),
			vec3(0.0f,roomHeight/2.0f -2.0f,roomWidth/2.0f-doorSize),
			vec3(0.0f,roomHeight/2.0f-2.0f,-roomWidth/2.0f+doorSize),
		};
		this->scales = {
			vec3((roomWidth-doorSize)/2.0f,roomHeight / 2.0f,1.0f),
			vec3((roomWidth-doorSize) / 2.0f,roomHeight / 2.0f,1.0f),
			vec3(1.0f,roomHeight / 2.0f,(roomWidth-doorSize) / 2.0f),
			vec3(1.0f,roomHeight / 2.0f,(roomWidth-doorSize) / 2.0f),
		};
	}
