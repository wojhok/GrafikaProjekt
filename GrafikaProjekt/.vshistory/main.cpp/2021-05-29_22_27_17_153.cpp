...
void drawScene(GLFWwindow* window, float position_z,float position_x, Camera camera) {
	...
	//{
	//	mat4 M1 = M;
	//	M1 = glm::translate(M, room.translates[i]);
	//	M1 = glm::rotate(M, room.rotates[i].angle, room.rotates[i].axis);
	...
	//	glDrawArrays(GL_TRIANGLES, 0, quadWalls[i].vertexCount); //Narysuj obiekt
